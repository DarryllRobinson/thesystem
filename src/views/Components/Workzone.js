...
class Workzone extends Component {
  ...
  getWorkspaceName(pathname) {
    ...
    if (indexOfFifth < 0)
      console.log('5: ', pathname.substring(indexOfThird + 1, indexOfFourth));
    if (indexOfFifth < 0)
      return pathname.substring(indexOfThird + 1, indexOfFourth);
    // For routes with /api/{resource}/{table}/:id pattern
    if (indexOfFourth < 0)
      console.log('4: ', pathname.substring(indexOfThird + 1, apiLength));
    if (indexOfFourth < 0)
      return pathname.substring(indexOfThird + 1, apiLength);
    // For routes with /api/{table}/:id pattern
    if (indexOfThird < 0)
      console.log('3: ', pathname.substring(indexOfSecond + 1, apiLength));
    if (indexOfThird < 0)
      return pathname.substring(indexOfSecond + 1, apiLength);
    // For routes with /api/{table} pattern
    if (indexOfSecond < 0)
      console.log('2: ', pathname.substring(indexOfFirst, indexOfSecond + 1));
  ...
  async loadRecords(currentStatus, task, workspace) {
    ...
    const type = this.props.location.state.type
      ...
      : sessionStorage.getItem('cwsType');
    ...
    if (this.props.history.location.state.records) {
      //console.log('props records: ', this.props.history.location.state.records);
      records = this.props.history.location.state.records;
    } else {
      records = await this.mysqlLayer.Get(
        `/${type}/${workspace}/list_all/${clientId}`
      );
    } //} //else { records = this.props.location.state.records; }
    /*if (!this.props.location.state.records || this.props.location.state.records.length === 0) { */ //console.log('records: ', records);
    ...
    if (records) {
      records.forEach((record) => {
        ...
        if (
          record.currentStatus === recordStatus &&
          ...
          const updatedDate = record.updatedDate
            ? moment(record.updatedDate).format('YYYY-MM-DD HH:mm:ss')
            : '';
            ...
            return (
              <div>
                ...
                <button
                  ...
                  onClick={() =>
                    this.loadRecords('Pended - Agent', this.state.workspace)
                  }
                >
                  Load Pended Applications
                </button>
                <button
                  type="button"
                  className="btn btn-secondary"
                  onClick={() =>
                    this.loadRecords(
                      'Decline Review - Agent',
                      this.state.workspace
                    )
                  }
                >
                  Load Decline Review Applications
                </button>
              </div>
            );
          case 'Pended - Store':
            return (
              <div>
                ...
                <button
                  ...
                  className="btn btn-secondary"
                  onClick={() =>
                    this.loadRecords('Referred', this.state.workspace)
                  }
                >
                  Load Referred Applications
                </button>
                <button
                  type="button"
                  className="btn btn-secondary"
            ...
            return (
              <div>
                ...
                <button
                  ...
                  className="btn btn-secondary"
                  onClick={() =>
                    this.loadRecords('Pended - Store', this.state.workspace)
                  }
                >
                  Load Pended Applications
                </button>
      ...
      case 'collections':
        switch (currentStatus) {
            ...
            return (
              <div>
                ...
                <button
                  type="button"
                  className="btn btn-secondary"
                  onClick={() => this.loadRecords('Open', this.state.workspace)}
                >
                  Load Open Cases
                </button>
                <button
                  ...
                  className="btn btn-secondary"
                  onClick={() =>
                    this.loadRecords('Closed', this.state.workspace)
                  }
                >
                  Load Closed Cases
                </button>
            ...
            return (
              <div>
                ...
                <button
                  ...
                  className="btn btn-secondary"
                  onClick={() =>
                    this.loadRecords('Pended', this.state.workspace)
                  }
                >
                  Load Pended Cases
                </button>
          ...
          case 'Locked':
            return (
              <div>
                <h4>{workspaceCapitalised}: Locked Cases</h4>
                <button
                  type="button"
                  className="btn btn-secondary"
                  onClick={() => this.loadRecords('Open', this.state.workspace)}
  ...
  render() {
    ...
    return (
      <div className="col-12">
        ...
        <MDBDataTable striped bordered small data={gridData} />
      </div>
    );
  }
}
