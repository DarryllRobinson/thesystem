...
class Collection extends Component {
  ...
  async componentDidMount() {
    ...
    await this.mysqlLayer
      .Get(`/${type}/${workspace}/read_item/${clientId}/${recordId}`)
      .then((response) => {
    ...
    await this.mysqlLayer
      .Get(`/${type}/${workspace}/read_outcomes/${clientId}/${recordId}`)
    ...
    await this.mysqlLayer
      .Get(`/${type}/${workspace}/read_contacts/${clientId}/${recordId}`)
    ...
    let transactionTypes = await this.mysqlLayer.Get(
      `/admin/transactiontypes/list_all`
    );
    let accountStatuses = await this.mysqlLayer.Get(
      `/admin/accountstatuses/list_all`
    );
    let cipcStatuses = await this.mysqlLayer.Get(
      `/admin/cipcstatuses/list_all`
    );
    ...
    const prevStatus =
      record.currentStatus !== undefined ? record.currentStatus : 'Open';
    ...
    await this.mysqlLayer.Put(
      `/${type}/cases/update_item/${clientId}/${recordId}`,
      update
    );
  }
  ...
  handleDate(e) {
    if (typeof e !== 'string') {
      const nextVisitDateTime = moment(e.toDate()).format(
        'YYYY-MM-DD HH:mm:ss'
      );
  ...
  handlePTPDate(e) {
    if (typeof e !== 'string') {
      ...
      const followup = moment(e.toDate())
        .subtract(1, 'day')
        .set({ hour: 8, minute: 0 })
        .format('YYYY-MM-DD HH:mm:ss');
      //var date = moment("1946-05-21").set({"hour": 15, "minute": 32});
      //console.log('followup: ', followup);
      this.setState({ ptpDate: ptpDate, nextVisitDateTime: followup });
    }
  }

  handleDebitDate(e) {
    if (typeof e !== 'string') {
      const debitResubmissionDate = moment(e.toDate()).format(
        'YYYY-MM-DD HH:mm:ss'
      );
  ...
  async cancel() {
    ...
    await this.mysqlLayer.Put(
      `/${this.state.type}/cases/update_item/${this.state.clientId}/${this.state.collection.caseId}`,
      update
    );
  ...
  checkFields(process, fields) {
    ...
    fields.forEach((field) => {
      stateConsts.forEach((stateConst) => {
        ...
        if (stateField === field) {
          if (stateValue === '')
            problems.push(
              `Please enter a value for ${_.startCase(stateField)}`
            );
        }
      });
    });
    ...
    if (this.state.emailUsed === null && this.state.transactionType === 'Email')
      problems.push('Please provide an email address');
    if (
      this.state.role !== 'kam' &&
      (!this.state.outcomeNotes || this.state.outcomeNotes.length < 10)
    )
      problems.push('Please enter a note longer than 10 characters');
    if (
      this.state.role === 'kam' &&
      (!this.state.kamNotes || this.state.kamNotes.length < 10)
    )
      problems.push('Please enter a KAM note longer than 10 characters');
    if (
      this.state.numberCalled === null &&
      this.state.transactionType === 'Call'
    )
      problems.push('Please provide a telephone number');
    if (this.state.numberCalled !== null && this.state.numberCalled.length > 11)
      problems.push('The telephone number can only be up to 11 digits long');
    if (process === 'Pended' && this.state.pendReason === '---')
      problems.push('Please select a pend reason');
    if (this.state.ptpDate && !this.state.ptpAmount)
      problems.push('Please provide a PTP amount');
    if (!this.state.ptpDate && this.state.ptpAmount)
      problems.push('Please provide a PTP date');
    if (this.state.debitResubmissionDate && !this.state.debitResubmissionAmount)
      problems.push('Please provide a debit resubmission amount');
    if (!this.state.debitResubmissionDate && this.state.debitResubmissionAmount)
      problems.push('Please provide a debit resubmission date');
  ...
  async processRecord(process) {
    ...
    const mandatoryFields = [
      ...
      {
        action: 'Updated',
        fields: ['currentAssignment', 'nextVisitDateTime', 'outcomeResolution'],
      },
    ];
    ...
    if (problems.length === 0) {
      ...
      if (!ptpDate && !debitResubmissionDate) {
        ...
        outcomeInsert = {
          ...
          closedDate: closedDate,
          closedBy: closedBy,
          debitResubmissionDate: moment(debitResubmissionDate).format(
            'YYYY-MM-DD'
          ),
      ...
      await this.mysqlLayer.Put(
        `/${type}/customers/update_item/${clientId}/${this.state.collection.customerRefNo}`,
        customerUpdate
      );
      await this.mysqlLayer.Put(
        `/${type}/accounts/update_item/${clientId}/${this.state.collection.accountNumber}`,
        accountUpdate
      );
      await this.mysqlLayer.Put(
        `/${type}/cases/update_item/${clientId}/${this.state.collection.caseId}`,
        caseUpdate
      );

      let resp = await this.mysqlLayer.Post(
        `/${type}/outcomes/create_item/${clientId}`,
        outcomeInsert
      );
  ...
  render() {
    if (this.state.collection === null) {
      ...
      let outcomesNotes = '';
      if (
        this.state.outcomeRecords.length > 0 &&
        this.state.outcomeRecords[0].outcomeNotes !== undefined
      ) {
        ...
        this.state.outcomeRecords.forEach((outcomeRecord, idx) => {
          ...
          outcomesNotesArray[idx] =
            `${idx + 1}: ` + outcomeRecord.outcomeNotes + '\n\r';
        });
      ...
      if (
        this.state.outcomeRecords.length > 0 &&
        this.state.outcomeRecords[0].outcomeResolution !== undefined
      ) {
        let outcomeArray = [];
        this.state.outcomeRecords.forEach((outcomeRecord, idx) => {
          let ptpDate = outcomeRecord.ptpDate
            ? moment(outcomeRecord.ptpDate).format('YYYY-MM-DD')
            : '--';
          let debitResubmissionDate = outcomeRecord.debitResubmissionDate
            ? moment(outcomeRecord.debitResubmissionDate).format('YYYY-MM-DD')
            : '--';

          outcomeArray[idx] =
            moment(outcomeRecord.createdDate).format('YYYY-MM-DD HH:mm:ss') +
            ' by user ' +
            outcomeRecord.createdBy +
            '\n' +
            'Transaction type: ' +
            outcomeRecord.transactionType +
            '\n' +
            'Contacted person: ' +
            outcomeRecord.contactPerson +
            '\n' +
            'Number called: ' +
            outcomeRecord.numberCalled +
            '\n' +
            'Email used: ' +
            outcomeRecord.emailUsed +
            '\n' +
            'PTP date: ' +
            ptpDate +
            '\n' +
            'PTP amount: R' +
            outcomeRecord.ptpAmount +
            '\n' +
            'Pend reason: ' +
            outcomeRecord.pendReason +
            '\n' +
            'Debit order resubmission date: ' +
            debitResubmissionDate +
            '\n' +
            'Debit order resubmission amount: R' +
            outcomeRecord.debitResubmissionAmount +
            '\n' +
            'Outcome resolution: ' +
            outcomeRecord.outcomeResolution +
            '\n' +
            'Next visit date and time: ' +
            moment(outcomeRecord.nextVisitDateTime).format(
              'YYYY-MM-DD HH:mm:ss'
            ) +
            '\n' +
            'Next steps: ' +
            outcomeRecord.nextSteps +
            '\n' +
            'Outcome notes: \n' +
            outcomesNotes +
            '\n' +
      ...
      pendList.push(
        this.state.pendReasons.map((pend) => (
          <option key={pend.id} value={pend.pendreason}>
            {pend.pendreason}
          </option>
        ))
      );
      ...
      //console.log('pendList before: ', pendList);
      transactionTypeList.push(
        this.state.transactionTypes.map((type) => (
          <option key={type.id} value={type.transactiontype}>
      ...
      const userList = [
        <option key="0" value={this.state.user}>
          Me
        </option>,
      ];
      userList.push(
      ...
      const accountStatusList = [
        <option key="0" value={this.state.collection.accountStatus}>
          {this.state.collection.accountStatus}
        </option>,
      ];
      accountStatusList.push(
        this.state.accountStatuses.map((accountStatus) => (
          <option key={accountStatus.id} value={accountStatus.accountStatus}>
            {accountStatus.accountStatus}
          </option>
        ))
      );

      const cipcStatusList = [
        <option key="0" value={this.state.collection.regIdStatus}>
          {this.state.collection.regIdStatus}
        </option>,
      ];
      cipcStatusList.push(
        this.state.cipcStatuses.map((cipcStatus) => (
          <option key={cipcStatus.id} value={cipcStatus.cipcStatus}>
            {cipcStatus.cipcStatus}
          </option>
        ))
      );
      ...
      idvStatusList.push(
        this.state.idvStatuses.map((idvStatus) => (
          <option key={idvStatus.id} value={idvStatus.idvStatus}>
            {idvStatus.idvStatus}
          </option>
        ))
      );
      ...
      let repNumber = this.state.contactRecords[0].representativeNumber
        ? `tel:${this.state.contactRecords[0].representativeNumber}`
        : '00000';
              ...
              <div className="card border-primary">
                ...
                <div className="card-body text-left">
                    ...
                    <div className="col-12">
                      <div className="form-group">
                        <label htmlFor="accountNotes">
                          Account Number {collection.accountNumber} - Notes
                        </label>
                        <textarea
                          disabled={true}
                          rows="3"
                  ...
                  <div className="row">
                    ...
                    <div className="col-4">
                      <div className="form-group">
                        <label htmlFor="customerAnalysis">
                          Customer Analysis
                        </label>
                        <input
                          disabled={true}
                  ...
                  <div className="row">
                    ...
                    <div className="col-4">
                      <div className="form-group">
                        <label htmlFor="lastPaymentAmount">
                          Last Payment Amount
                        </label>
                        <NumberFormat
                          disabled={true}
                  ...
                  <div className="row">
                    <div className="col-4">
                      <div className="form-group">
                        <label htmlFor="nextVisitDateTime">
                          Next Visit Date and Time
                        </label>
                        <input
                          disabled={true}
                    ...
                    <div className="col-4">
                      <div className="form-group">
                        ...
                        <input
                          ...
                          className="form-control"
                          value={
                            this.state.contactRecords[0].representativeName ||
                            ''
                          }
                        />
                      </div>
                    ...
                    <div className="col-4">
                      <div className="form-group">
                        <label htmlFor="representativeNumber">
              ...
              <div className="card border-primary">
                ...
                <div className="card-body text-left">
                  ...
                  <div className="row">
                    <div className="col-4">
                      <div className="form-group">
                        <label htmlFor="debitResubmissionDate">
                          Debit Resubmission Date
                        </label>
                        <DateTime
                          isValidDate={valid}
                    ...
                    <div className="col-4">
                      <div className="form-group">
                        <label htmlFor="debitResubmissionAmount">
                          Debit Resubmission Amount
                        </label>
                        <NumberFormat
                          displayType={'input'}
                          className="form-control"
                          ...
                          onValueChange={(values) => {
                            this.setState({
                              debitResubmissionAmount: values.floatValue,
                            });
                          }}
                    ...
                    <div className="col-4">
                      <div className="form-group">
                        ...
                        <DateTime
                          ...
                          value={this.state.nextVisitDateTime}
                        />
                      </div>
                    </div>
