...
class ExcelReader extends Component {
  ...
  handleChange(e) {
    this.setState({ errors: [], loading: false, compliance: '' });
  ...
  /*async saveRecordsToDatabase(workspace, data) {
    console.log('data: ', data);
    let fields;
    mappingFields.forEach((field) => {
      if (field.workspace === workspace) {
        fields = field.entity[1].fields;
      }
    });
  ...
  async saveCustomerRecordsToDatabase(recordChunks) {
    ...
    for (let i = 0; i < recordChunks.length; i++) {
      ...
      const response = await this.postToDb(customers, 'customers').then(
        this.setState({
          progress: Math.round(((i + 1) / recordChunks.length) * 100),
        })
      );
    ...
    this.setState({ progress: 100, uploaded: { customers: true } });
    return totalRecords;
  }
  ...
  async saveAccountRecordsToDatabase(recordChunks) {
    ...
    for (let i = 0; i < recordChunks.length; i++) {
      ...
      const response = await this.postToDb(accounts, 'accounts').then(
        this.setState({
          progress: Math.round(((i + 1) / recordChunks.length) * 100),
        })
      );
  ...
  async saveCaseRecordsToDatabase(recordChunks) {
    ...
    for (let i = 0; i < recordChunks.length; i++) {
      ...
      const response = await this.postToDb(cases, 'cases').then(
        this.setState({
          progress: Math.round(((i + 1) / recordChunks.length) * 100),
        })
      );
  ...
  async saveOutcomeRecordsToDatabase(recordChunks) {
    ...
    for (let i = 0; i < recordChunks.length; i++) {
      ...
      const response = await this.postToDb(outcomes, 'outcomes').then(
        this.setState({
          progress: Math.round(((i + 1) / recordChunks.length) * 100),
        })
      );
  ...
  async saveContactRecordsToDatabase(recordChunks) {
    ...
    for (let i = 0; i < recordChunks.length; i++) {
      ...
      let response = await this.postToDb(contacts, 'contacts').then(
        this.setState({
          progress: Math.round(((i + 1) / recordChunks.length) * 100),
        })
      );
  ...
  render() {
    ...
    const workspaceMessages = this.state.workspaces.map((workspace, idx) => (
      <div key={idx} className="container">
        <div className="row">
          <div className="col-8">
            <label htmlFor="message">Message for {workspace}</label>
            <br />
            <br />
            <br />
          </div>
