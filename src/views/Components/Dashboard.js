...
class Dashboard extends Component {
  ...
  async prepareClientDashboard(client) {
    ...
    const clientservices = await this.mysqlLayer.Get(
      `/admin/clientservices/${client}`
    );
    ...
    services.forEach(async (service) => {
      ...
      await this.mysqlLayer
        .Get(`/client/${workspace}/list_all/${client}`)
        .then(async (records) => {
  ...
  async prepareAgentDashboard(client) {
    ...
    services.forEach(async (service) => {
      ...
      await this.mysqlLayer
        .Get(`/${type}/${workspace}/list_all/${client}`)
        .then(async (records) => {
          worklists = await this.getWorklists(workzones, records);
          let workspaces = [];
          workspaces.push({
          ...
          switch (task) {
            ...
            case 'list_today':
              ...
              if (
                ...
                moment(record.nextVisitDateTime).format('YYYY-MM-DD') >
                  moment(new Date() - 86400000).format('YYYY-MM-DD')
              ) {
                ++count;
                tags.push(task);
        ...
        records.forEach((record) => {
          ...
          switch (task) {
            ...
            case 'list_today':
              //console.log('today: ', moment(new Date() - 86400000).format('YYYY-MM-DD'), moment(record.nextVisitDateTime).format('YYYY-MM-DD'));
              if (
                moment(record.closedDate).format('YYYY-MM-DD') >
                moment(new Date() - 86400000).format('YYYY-MM-DD')
              ) {
                ++count;
                tags.push(task);
            ...
            default:
              console.log(
                'task switch default - there must be a problem. ',
                task
              );
          }
        ...
        switch (role) {
          case 'agent':
            ...
            lists.forEach((list) => {
              ...
              if (recordStatuses.find((recordStatus) => list === recordStatus))
                finalList.push(list);
            });
